...
class Smallnet(nn.Module):
    ...
    def __init__(self,img_rows, img_cols, n_patients):
        ...
        t.nn.init.uniform(self.conv1.weight,-1,1)
        ...
        t.nn.init.uniform(self.conv2.weight,0,1)
        ...
        t.nn.init.uniform(self.fc1.weight,-1,1)
        ...
        t.nn.init.uniform(self.patient.weight,0,1)
        ...
        t.nn.init.uniform(self.tumorclass.weight,-1,1)
...
#                s_loss = loss(input = p_class,target = t_class)
#                print(model.named_parameters())
                
                negative = ['bn1.weight', 'bn1.bias', 'bn1.running_mean', 'bn1.running_var', 'bn1.num_batches_tracked', 
                            'bn2.weight', 'bn2.bias', 'bn2.running_mean', 'bn2.running_var', 'bn2.num_batches_tracked',
                            'conv1.weight', 'conv2.weight', 'fc1.weight', 'fc1.bias']
                
                
                # TODO: iterate over all state_dict and adjust gradient. Set negative for all layers but last of moa
                
                tmp_parameters = deepcopy(model.parameters)
                
                class_loss = loss(input = p_class, target = t_class)
                class_loss.backward()
                for old, tmp in zip(model.parameters(),tmp_parameters()):
                    if tmp.requires_grad and tmp.grad is not None:
                        tmp.grad = old.grad
                print(model.state_dict()['bn1.weight'].grad)
                optimizer.zero_grad()
                id_loss = loss(input = p_id, target = t_id)
                for old, tmp in zip(model.parameters(),tmp_parameters()):
                    if tmp.requires_grad and tmp.grad is not None:
                        
                        old.grad = 0.5*(-old.grad + tmp.grad)
